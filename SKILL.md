...
| `--type` / `-t` | all five | Comma-separated: `book,movie,tv,music,game` (`film-tv` expands to `movie,tv`) |
...
- Douban files films, TV series and anime under one API category; this tool splits
  them into `movies.md` and `tv.md` by `subject.subtype`. Both share a single fetch,
  so asking for one or both costs the same.
